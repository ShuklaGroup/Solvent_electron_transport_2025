...
def plot_1d_kde(feats,solv,bond_type,color,weights=None,sys='MAAM'):
    ...
    plt.xlabel("1-4 OH-bond Distance (Å)")
    ...
    plt.tight_layout()
    plt.savefig(f'plots/HO_plots/{sys}_{solv}_{bond_type}_OH.png',dpi=300)


def find_kde_peaks(feats, weights=None, bw_adjust=0.8):
    """
    Finds peak positions in the 1D KDE from given features.

    Parameters:
    - feats: list of arrays, concatenated into a single 1D array
    - weights: optional array of weights for each data point
    - bw_adjust: KDE bandwidth adjustment factor

    Returns:
    - peaks: list of (x_position, density) tuples
    """
    data = np.concatenate(feats).squeeze() * 10  # Convert nm to Å
    ...
    # Generate KDE and extract x/y
    kde_plot = sns.kdeplot(x='data', weights='weights', data=df, bw_adjust=bw_adjust)
    line = kde_plot.get_lines()[0]
    x_vals = line.get_xdata()
    y_vals = line.get_ydata()
    plt.close()  # Prevent actual plot from showing
    
    # Find peaks
    peak_indices, _ = find_peaks(y_vals)
    peaks = list(zip(x_vals[peak_indices], y_vals[peak_indices]))
    return peaks

feats = pickle.load(open("traj_feats/feats_HO_water.pkl", "rb"))
peaks = find_kde_peaks(feats)
print("KDE Peak Positions (Å) and Heights:")
for x, y in peaks:
    print(f"x = {x:.2f} Å, density = {y:.3f}")
...
for s,mol in zip(solv,solv_mol):
    ...
    for rep in tqdm(range(16)):
        parm_path = f'../{s}/MAAM/parm7/MAAM_{rep}.parm7' 
        traj_path = f'../{s}/MAAM/sim/prod/MAAM_{rep}.dcd'
        ...
        # print(traj.topology.select('name O and resid 0'))
        # print(traj.topology.select('name H and resid 3'))
        residues = list(traj.topology.residues)
        for res in residues:
            atom_names = [atom.name for atom in res.atoms]
            #print(atom_names)
            # print(f"Atom names in residue {res.name} ({res.index}): {atom_names}")
            # sys.exit()
        _, psi = md.compute_psi(traj)
        _, phi = md.compute_phi(traj)
        O_i_H_i3 = np.hstack((traj.topology.select('name O and resid 0'), traj.topology.select('name H and resid 3'))).reshape(-1, 2)
        H_O_distance = md.compute_distances(traj, O_i_H_i3)
        tot_feats = np.hstack((psi,phi))#,H_O_distance))
        feats.append(tot_feats)
        H_O.append(H_O_distance)

    pickle.dump(feats,open(f'traj_feats/feats_{s}.pkl','wb')) 
    pickle.dump(H_O,open(f'traj_feats/feats_HO_{s}.pkl','wb'))

# is there a way to find values at all the peaks for these 1d kde plots?
...
for s,c in zip(solv,colors):
    feat = pickle.load(open(f'traj_feats/feats_HO_{s}.pkl','rb'))
    plot_1d_kde(feat,s,bond_type='1 to 4',color=c)
...
for s in solv:
    ...
    tic_lagtime = 10
    feat_list = pickle.load(open(f'traj_feats/feats_{s}.pkl','rb'))
    ...
    plt.savefig(f'plots/tic_plots/{s}_tics.png',dpi=300)
    plt.close()



## BOOTSTRAPPING FOR TICA

fraction_list = [0.7,0.8,0.9,1.0]  # 80% random subset

for s in solv:
    for fraction in fraction_list:
        feats = pickle.load(open(f'traj_feats/feats_{s}.pkl', 'rb'))
        tica = TICA(lagtime=10).fit_fetch(feats)
        proj = [tica.transform(f)[np.random.choice(len(f), int(len(f)*fraction), replace=False)] for f in feats]
        concat = np.concatenate(proj)
        
        e = energy2d(concat[:,0], concat[:,1], bins=(50,50), kbt=2.479/4.184, shift_energy=True)
        z1lim = [0, 5]
        levels = np.linspace(*z1lim, 20)
        plt.figure(figsize=(8,6))
        ax, contour, cbar = e.plot(contourf_kws=dict(cmap=anh_cmap), levels=levels)
        ticks = np.linspace(*z1lim, 6)
        cbar.ax.set_yticks(ticks); cbar.ax.set_yticklabels(ticks)
        cbar.ax.tick_params(axis='y'); cbar.set_label('Free Energy (kcal/mol)')
        ax.tick_params(axis='both'); ax.set_xlabel('tIC-1'); ax.set_ylabel('tIC-2')
        plt.tight_layout()
        plt.savefig(f'plots/tic_plots/fractions/{s}_{fraction}_tics.png', dpi=300)
        plt.close()
...
for s in solv:
    ...
    feat_list = pickle.load(open(f'traj_feats/feats_{s}.pkl','rb'))
    ho_feats = pickle.load(open(f'traj_feats/feats_HO_{s}.pkl','rb'))
    ...
    cmap = plt.cm.get_cmap('plasma')  # Get the existing colormap
    cmap.set_bad('white')  # Set the color for NaN values to white

    # Transpose masked_average_metric to match the meshgrid
    contour = ax.contourf(X, Y, masked_average_metric.T, levels=levels, cmap=cmap)  # Transpose for correct orientation
    ...
    cbar.set_label('1-4 OH-bond distance (Å)')  # Update label to reflect the metric used
    ...
    #plt.title(f"{sys} in {solv}")
    plt.savefig(f'plots/tic_plots/{s}_tic_dis.png', dpi=300)